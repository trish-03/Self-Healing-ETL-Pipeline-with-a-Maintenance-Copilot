...
function DashboardContent() {
  ...
  // Generate interactive trend timeline plotting degradation to resolution
  ...
  useEffect(() => {
    if (health) {
      ...
      if (health.status === 'FRAGMENTED') {
        // Mocking a step-wise build up over 50 batches leading to this current state
        setChartData([
        ...
        // Optimized scenario mapping clean file compaction
        setChartData([
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-[#0b0f17] text-slate-100">
      ...
      <aside className={`bg-[#111827] border-r border-slate-800 flex flex-col justify-between shrink-0 transition-all duration-300 relative ${isSidebarCollapsed ? 'w-20' : 'w-64'}`}>
        <button
        ...
        <div>
          <div className={`p-6 border-b border-slate-800 flex items-center gap-3 ${isSidebarCollapsed ? 'justify-center' : ''}`}>
            ...
            {!isSidebarCollapsed && (
              <div>
                <h1 className="font-extrabold text-sm tracking-wide text-white">LAKEHOUSE COPILOT</h1>
                <p className="text-[10px] text-slate-400 font-medium tracking-wider uppercase">Project 2 Control Plane</p>
              </div>
            )}
          </div>
          ...
          <nav className="p-4 space-y-1.5">
            <button
              ...
              className={`w-full flex items-center rounded-lg text-xs font-semibold tracking-wide transition ${isSidebarCollapsed ? 'justify-center p-2.5' : 'px-4 py-2.5 gap-3'} ${currentView === 'dashboard' ? 'bg-[#226b4d] text-white shadow' : 'text-slate-400 hover:bg-slate-800'}`}
            >
            ...
            <button
              ...
              className={`w-full flex items-center rounded-lg text-xs font-semibold tracking-wide transition ${isSidebarCollapsed ? 'justify-center p-2.5' : 'px-4 py-2.5 gap-3'} ${currentView === 'metrics' ? 'bg-[#226b4d] text-white shadow' : 'text-slate-400 hover:bg-slate-800'}`}
            >
              <Activity size={16} />
            ...
            <button
              ...
              className={`w-full flex items-center rounded-lg text-xs font-semibold tracking-wide transition ${isSidebarCollapsed ? 'justify-center p-2.5' : 'px-4 py-2.5 gap-3'} ${currentView === 'full_chat' ? 'bg-[#226b4d] text-white shadow' : 'text-slate-400 hover:bg-slate-800'}`}
            >
      ...
      <main className="flex-1 flex flex-col min-w-0 relative overflow-hidden">
        <header className="h-16 border-b border-slate-800 bg-[#111827]/50 flex items-center justify-between px-8 shrink-0 z-10">
          <div className="flex items-center gap-4">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Target Iceberg Catalog Table:</span>
            <div className="flex bg-[#0b0f17] p-1 rounded-lg border border-slate-800">
              {['fact_orders', 'fact_order_items'].map((table) => (
                <button
                  key={table}
                  onClick={() => setActiveTable(table)}
                  className={`px-3 py-1 rounded-md text-[10px] font-bold uppercase tracking-widest transition ${activeTable === table ? 'bg-slate-800 text-white shadow border border-slate-700' : 'text-slate-500 hover:text-slate-300'}`}
                >
                  {table.replace('_', ' ')}
                </button>
              ))}
        ...
        <div className="flex-1 overflow-y-auto p-8">
          {currentView === 'dashboard' && (
            <div className="space-y-6">
              {/* Upgraded 4-way Health Indicators */}
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-[#111827] border border-slate-800 p-5 rounded-xl flex items-center justify-between">
                  <div>
                ...
                <div className="bg-[#111827] border border-slate-800 p-5 rounded-xl">
                  ...
                  <span className="text-2xl font-black text-white tracking-tight">{isLoading ? '...' : health?.metrics.live_file_count}</span>
                </div>
                <div className="bg-[#111827] border border-slate-800 p-5 rounded-xl">
                  <span className="text-[10px] font-bold tracking-widest text-slate-500 uppercase block mb-1">Dangling MoR Deletes</span>
                  <span className={`text-2xl font-black tracking-tight ${health?.metrics.delete_file_count && health.metrics.delete_file_count > 0 ? 'text-red-400' : 'text-slate-400'}`}>
                ...
                <div className="bg-[#111827] border border-slate-800 p-5 rounded-xl">
                  ...
                  <span className="text-2xl font-black text-slate-300 tracking-tight">{isLoading ? '...' : health?.metrics.metadata_json_count}</span>
                </div>
              </div>

              {/* Functional Analytics Chart Container */}
              <div className="bg-[#111827] border border-slate-800 h-96 rounded-xl p-6 flex flex-col">
                <div className="mb-4">
                  <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest">Small-File Frag & Position Delete Accumulation Trend</h3>
                  <p className="text-[11px] text-slate-500 mt-0.5">Visually monitors write amplification scaling against optimization operations</p>
                </div>
          ...
          {currentView === 'metrics' && (
            <div className="bg-[#111827] border border-slate-800 p-6 rounded-xl text-slate-400 text-xs space-y-3">
              ...
              <ul className="list-disc pl-5 space-y-1 text-slate-300 font-mono text-[11px]">
                ...
                <li>Total Monitored Volumetric Mass: {((health?.metrics.total_size_bytes ?? 0) / 1024).toFixed(2)} KB</li>
      ...
      <div className={`fixed top-0 right-0 h-full w-96 bg-[#111827] border-l border-slate-800 shadow-2xl transform transition-transform duration-300 ease-in-out flex flex-col justify-between z-50 ${isCopilotOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="p-4 border-b border-slate-800 flex items-center justify-between bg-[#111827]/80 shrink-0">
          ...
          <button onClick={() => setIsCopilotOpen(false)} className="text-slate-400 hover:text-white p-1 rounded-md transition hover:bg-slate-800">
            <X size={16} />
          </button>
        </div>
